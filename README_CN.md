...
- **复制等级粘贴**：等级安装模式会粘贴已复制机器的工厂等级，不再将目标直接升到可用最高等级
- **AE2 与 QIO 联动**：从绑定网络中抽取等级安装器和升级模块
- **能量系统**：最多存储 200,000 FE；每次等级升级消耗 1,000 FE
- **创造模式等级粘贴**：忽略安装器和能量要求，并且不消耗任何资源
...
- **直升终极**：右键无等级、基础、高级或精英机器，直接升级到标准终极等级
- **硬性上限**：绝不会升级到无限、创造、悖论等终极以上扩展等级
- **只消耗电量**：最多储存 200,000 FE，每台成功升级的机器消耗 1,000 FE
- **无需安装器材料**：不需要也不消耗基础、高级、精英或终极等级安装器
- **工具不消耗**：终极等级安装器本身可以永久重复使用
- **不绑定网络**：不再绑定 AE2 或 QIO，只使用工具自身储存的电量
- **批量升级**：按住选区键左键两个角点，或按住 FTB Ultimine 连锁键右键
...
2. **充电**：放入 Mekanism 能量立方或兼容充电器中（每台成功升级的机器消耗 1,000 FE）
3. **执行升级**：右键低等级机器，直接升级到标准终极等级
4. **批量升级**：按住选区键左键两个角点，或按住 FTB 连锁键右键机器
5. **等级上限**：终极或任何更高扩展等级的机器保持不变，不需要任何等级安装器材料
